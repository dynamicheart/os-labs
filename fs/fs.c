...
file_block_walk(struct File *f, uint32_t filebno, uint32_t **ppdiskbno, bool alloc)
{
	...
	if (filebno < NDIRECT) {
		*ppdiskbno = &f->f_direct[filebno];
		...
		// Allocate the indirect block if it doesn't yet exist
		if (!f->f_indirect) {
			if (!alloc)
				return -E_NOT_FOUND;
			else {
...
file_get_block(struct File *f, uint32_t filebno, char **blk)
{
	...
	if (filebno < NDIRECT) {
		...
		*blk = diskaddr(f->f_direct[filebno]);
	} else {
		// Allocate the indirect block if it doesn't yet exist
		if (!f->f_indirect) {
			if ((r = alloc_block()) < 0)
		...
		if (!indirect[filebno - NDIRECT]) {
			if ((r = alloc_block()) < 0)
				return r;
			indirect[filebno - NDIRECT] = (uint32_t)r;
		}

		*blk = diskaddr(indirect[filebno - NDIRECT]);
	}
